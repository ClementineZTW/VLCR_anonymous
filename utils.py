...
class TokenLabelConverter(object):
    ...
    def char_encode_CTC(self,text):
        """ convert text-label into text-index but GO and SPACE excluded.
        """
        batch_len = torch.LongTensor(len(text),1).fill_(0)
        batch_text = torch.LongTensor(len(text),self.batch_max_length).fill_(self.dict[self.GO])
        for i,t in enumerate(text):
            batch_len[i] = torch.LongTensor([len(t)])
...
def get_args(is_train=True):
    ...
    parser.add_argument('--MultiCELoss', type=int, default=0, help='Assume your model outputs multiple times of string target, default=1.')
    parser.add_argument('--BCN',action='store_true', default=False, help='Use BCN for the lingual modeling. default=False')
    parser.add_argument('--amp',action='store_true', default=False, help='Train with AMP.')
    """ Data processing """
    ...
    """ Model Architecture """
    parser.add_argument('--Transformer', type=str, help='Transformer stage. mgp-str|char-str|SVTR|General_DeiT')

    #choices = ["mgp_str_base_patch4_3_32_128", "mgp_str_tiny_patch4_3_32_128", 
    #            "mgp_str_small_patch4_3_32_128", "char_str_base_patch4_3_32_128","SVTR-tiny",'General_DeiT_Tiny']
    parser.add_argument('--TransformerModel', default='', help='Which mgp_str transformer model')
    parser.add_argument('--Decoder',default='',help='Which decoder to use with General Deit Model')
    ...
    # For CRM
    parser.add_argument('--CRM',action='store_true',default=False,help='Use CRM Model.')
    parser.add_argument('--backbone_type', default='', help='Which DeiT backbone to use. Tiny|Small|Base|TinyShort')
    parser.add_argument('--load_from_DeiT', action='store_true', default=False, help='If load weight from DeiT.')
    parser.add_argument('--definition_string',type=str,help='Definition of decoder V&L sequence.')
    parser.add_argument('--supervised_blocks',type=str,help='Which blocks needs to be supervised.')
    parser.add_argument('--decoder_embed_dim',type=int,default=None,help='Specify the Decoder\s embed dim ,not supported.')
    parser.add_argument('--decoder_num_heads',type=int,default=None,help='Specify the Decoder\s num_heads.')
    parser.add_argument('--with_cls_token',action='store_true',default=False,help='Do not concat cls_token with query before cross attention.')
    parser.add_argument('--random_mask',action='store_true',default=False,help='Randomly mask in querys.')
    parser.add_argument('--share_parameters',action='store_true',default=False,help='Share parameters in blocks of the same class in decoder.')
    parser.add_argument('--SE',action='store_true',default=False,help='Apply SE Module to feature map instead of Linear.')
    parser.add_argument('--SVTRPatchEmbed',action='store_true',default=False,help='Apply SVTR Patch Embed to image instead of ViTs.')
    # For Predator
    parser.add_argument('--Predator',action='store_true',default=False,help='Use Predator.')
    #backbone_type shared with CRM

    # For Finder
    parser.add_argument('--Finder',action='store_true',default=False,help='Use Finder.')
    ...
    parser.add_argument('--T_0',type=int,default=1000000,help='Set T0 for cosineanneling scheduler.')
