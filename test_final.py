...
def benchmark_all_eval(model, criterion, converter, opt): #, calculate_infer_time=False):
    ...
    print('Total  evaluation data number: ',total_evaluation_data_number)
    print(f'Total forward_time: {total_forward_time:.06f} seconds')
    ...
    for i, (image_tensors, labels, imgs_path) in enumerate(evaluation_loader):
        ...
        if opt.Transformer in ["mgp-str"]:
            ...
            if opt.MultiCELoss != 0:
                cost = criterion(char_preds,target)
                M = opt.MultiCELoss #3
                T = opt.batch_max_length+2 #27
                L = M
                char_preds = char_preds[:,(L-1)*T:L*T,:]
                #(n,l,c) = char_preds.shape
                #char_preds = char_preds.reshape(n,M,T,c)[:,:,:,:]
                #char_preds = torch.mode(char_preds,dim=1)[0]
            else:
                cost = criterion(char_preds.contiguous().view(-1, char_preds.shape[-1]), target.contiguous().view(-1))
            ...
            char_pred_index = char_pred_index.view(-1, converter.batch_max_length-2)
            length_for_pred = torch.IntTensor([converter.batch_max_length-2] * batch_size).to(device)
            char_preds_str = converter.char_decode(char_pred_index, length_for_pred)
            for i,t in enumerate(char_preds_str):
                char_preds_str[i] = t.replace('[GO]','')
            char_pred_prob = F.softmax(char_preds, dim=2)
            char_pred_max_prob, _ = char_pred_prob.max(dim=2)
            char_preds_max_prob = char_pred_max_prob
            ...
            for index,gt in enumerate(labels):
                ...
                if char_pred == gt:
                    char_n_correct += 1
                char_pred_max_prob = char_pred_max_prob
                try:
                    char_confidence_score = char_pred_max_prob.cumprod(dim=0)[-1]
                except:
                    char_confidence_score = 0.0
...
def test(opt):
    ...
    model.eval()
    '''
    decoder = model.module.model.decoder
    print(decoder)
    v = decoder.blocks[0]
    l = decoder.blocks[1]
    decoder.blocks = torch.nn.Sequential(v,l,v,l,v,l,v,l,v)
    '''
    opt.eval = True
    with torch.no_grad():
